...
func (app *application) readJSON(w http.ResponseWriter, r *http.Request, dst any) error {
	...
	if err != nil {
		...
		case strings.HasPrefix(err.Error(), "json: unknown field "): // Decode() returns error in format below
	...
	err = dec.Decode(&struct{}{}) // empty anonymous struct checking for additional JSONs
...
func (app *application) readDate(qs url.Values, key string, defaultValue time.Time, v *validator.Validator) time.Time {
	...
	if err != nil {
		v.AddError(key, "must be a valid date value (YYYY-MM-DD)")
		return defaultValue
	}
	return t
}
